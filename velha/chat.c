...
void imprimir_tabuleiro(char tabuleiro[3][3]) { // imprimir tabuleiro
    printf("\n  1 2 3\n");
    printf(" -------\n");
    for (int i = 0; i < 3; i++) {
        printf("%d|", i + 1);
...
char verificar_vencedor(char tabuleiro[3][3]) { // Verificar se a vencedores
    for (int i = 0; i < 3; i++) { // verifica se a vencedor por linha
        if (tabuleiro[i][0] == tabuleiro[i][1] && tabuleiro[i][0] == tabuleiro[i][2] && tabuleiro[i][0] != ' ') {
            return tabuleiro[i][0];
        }
        if (tabuleiro[0][i] == tabuleiro[1][i] && tabuleiro[0][i] == tabuleiro[2][i] && tabuleiro[0][i] != ' ') { // verifica se a vencedor por coluna
...
int main() {
    ...
    if (jogador_atual == 0) {
        ...
        printf("O computador começa jogando.\n");
    }
    ...
    while (vencedor == ' ') {
        ...
        if (jogador_atual == 0 || (jogador_atual == 1 && letra_computador == jogador2)) {
            int linha, coluna;
